...
void UART2_Init(uint32_t baudrate) {
	...
	// ========================================================================
	// FLUSH ANY GARBAGE DATA IN RX BUFFER
	// ========================================================================

	// Give hardware time to stabilize
	    for (volatile int i = 0; i < 1000; i++);

	    // Clear any spurious RXNE/ORE flags by reading SR and DR
	    if (USART2->SR & (1 << 5)) {  // If RXNE set
	        volatile uint32_t temp = USART2->DR;  // Read DR (clears RXNE)
	        (void)temp;
	    }

	    // Clear overrun error if present
	    if (USART2->SR & (1 << 3)) {  // If ORE set
...
uint8_t UART2_DataAvailable(void) {
    // ========================================================================
    // NON-BLOCKING CHECK: IS DATA READY?
    // ========================================================================

    // Check RXNE flag without waiting
    // Returns 1 if data available, 0 if not
    // Allows main loop to do other tasks while waiting for data
